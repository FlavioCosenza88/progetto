...
async function watch ([cwd], {
  ...
  if (process.env.CDS_TYPESCRIPT) {
    const tsNodeRegister = localOrGlobal('ts-node', 'register')
    env.NODE_OPTIONS = `-r ${tsNodeRegister} ` + (env.NODE_OPTIONS || '')
  }
...
function localOrGlobal (id, pathInMod) {
  // require package.json to get a stable root path of the moduloe, not its main script
  const modPath = require.resolve (id+'/package.json', { paths:[ process.cwd(), npmGlobalModules() ] })
  return resolve(modPath, '..', pathInMod)
}
...
if (!module.parent) (async()=>{ // launched by nodemon
  ...
  try {
    ...
    default: if (!e.messages) { // compiler messages have been written by cli already
