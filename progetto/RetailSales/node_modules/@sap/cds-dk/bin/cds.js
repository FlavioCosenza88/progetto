...
const cli = { //NOSONAR
  ...
  async exec (cmd = process.argv[2], ...argv) {
    ...
    DEBUG?.(`[cds] - @sap/cds ${require('../lib/cds').version} loaded: ${require('../lib/cds').home}`)
    ...
    if (task) {
      ...
      try {
        ...
        process.exitCode = 1
        if (e.messages) require('../lib/cds')._log (e.messages)
  ...
  load (cmd,_default) {
    if (cmd === 'build/all') return _require ('./'+cmd) // TODO: remove with CDS 7
