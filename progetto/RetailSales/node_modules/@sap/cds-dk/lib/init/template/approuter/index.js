...
const { OPTION_XSUAA } = require('../../constants')
const {
...
module.exports = class ApprouterTemplate extends require('../templateBase') {
    ...
    async run() {
        ...
        const appPath = projectDescriptor.ui.appPath ?? 'app'
    ...
    async runDependentMerging(context = {}) {
        const projectDescriptor = await readProject(this.options)
        const { isNodejs, isJava, isExtensible, isMultitenant, hasMta, hasHelm, srvPath } = projectDescriptor.cap
        ...
        if (hasMta) {
            ...
            await mergeYAML('mta.yaml', join(__dirname, 'files', 'mta.yaml.hbs'), projectDescriptor, {
                additions,
                relationships: [{
                    insert: [xsuaa, 'name'],
        ...
        const xsAppPath = join(projectDescriptor.ui.appPath ?? 'app', 'xs-app.json')
        const additions = isExtensible ? [{ ...approuterExtensibility, at: 0 }] : []
        await mergeJSON(xsAppPath, join(__dirname, 'files', 'xs-app.json.hbs'), projectDescriptor, { additions })
    }
}
