...
const { OPTION_JAVA, URLS } = require('../../constants');
...
module.exports = class HanaTemplate extends require('../templateBase') {
    ...
    async run() {
        ...
        const { isJava, configFile } = projectDescriptor.cap
        projectDescriptor.cap.shortcut = !(await read(configFile)).cds?.requires?.db?.kind

        const env = await this.getEnv() // REVISIT: Get info from projectDescriptor
    ...
    async runDependentMerging(context = {}) {
        const projectDescriptor = await readProject(this.options)
        const { hasMta, hasHelm, hasHtml5Repo, isJava, isMultitenant, srvPath } = projectDescriptor.cap
