...
module.exports = class ParserContext {
    ...
    getServicesFromContent(content) {
        ...
        Object.entries(content.components.messages).forEach(([key, value]) => {
            ...
            const regexGroups = typeRegex.exec(key);
            if (regexGroups == null) {
                throw new Error(messages.INVALID_MESSAGE_NAME);
            }

            const service = regexGroups[1] + "." + regexGroups[2];
            const event = regexGroups[0];

            // add service to services entry if not present
            if (!(service in services)) {
                services[service] = { 'kind': 'service', ...serviceAnnotations, 'events': {}, 'types': {} };
            }
            ...
            services[service].events[event] = { 'kind': 'event', ...eventAnnotations, elements: properties };
        });
    ...
    getAnnotations(content, annotationsMapping) {
        ...
        for (const [key, value] of Object.entries(content)) {
            ...
            else if (key.startsWith('x-sap-')) {
                if (!annotations['@AsyncAPI.Extensions']) {
                    annotations['@AsyncAPI.Extensions'] = {};
                }
                annotations['@AsyncAPI.Extensions'][key] = value;
            }
        }
    ...
    parseProperty(eventName, propertyName, propertyContent, isRequired) {
        ...
        if (type && (type === 'object' || type === 'array')) {
            ...
            types[typeName] = { 'kind': 'type', ...elements };
