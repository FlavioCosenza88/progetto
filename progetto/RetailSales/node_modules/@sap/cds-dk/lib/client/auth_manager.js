...
function getTokenUrl(params) {
    ...
    if (params.has('clientsecret')) {
        ...
        tokenUrl.search += `&clientid=${encodeURIComponent(params.get('clientid'))}&key=${encodeURIComponent(params.get('key'))}`;
    }
...
async function retrieveTokenOrPasscodeUrl(params) {
    ...
    do {
        ...
        DEBUG?.(`Getting authentication token or passcode URL from ${(params.obfuscateQueryParams(tokenUrl))}`);
